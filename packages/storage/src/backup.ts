...
export interface StorageBackup {
  readonly schemaVersion: "vera.storage-backup/v1";
...
export async function exportStorageBackup(
  ...
  const hashInput = {
    schemaVersion: "vera.storage-backup/v1" as const,
    exportedAt,
    evaluationRuns: (await prisma.evaluationRunRecord.findMany({ orderBy: { id: "asc" } })).map(
      ({ payload }) => payload as JsonValue,
    ),
    reviewDecisions: (
      await prisma.reviewDecisionRecord.findMany({
        orderBy: [{ runId: "asc" }, { sequence: "asc" }],
      })
    ).map(({ payload }) => payload as JsonValue),
    blobObjects: (await prisma.blobObject.findMany({ orderBy: { sha256: "asc" } })).map(
      (record) =>
        ({
